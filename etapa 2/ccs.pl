...
at([N, S, E, W, NC], Direction, What) :- Direction = n, What = N; % nord
                                        Direction = s, What = S; % sud
                                        Direction = e, What = E; % est
                                        Direction = w, What = W. % vest
...
atL(Tile, [], What).
atL(Tile, [D | Directions], What) :- at(Tile, D, What), % verificare pt prima directie
                                    atL(Tile, Directions, What).  % verificare pt restul listei
...
hasTwoCitadels([N, S, E, W, NC]) :- NC = 2.
...
ccw(Tile, 0, Tile). % piesa nerotita
ccw([N, S, E, W, NC], 1, [E, W, S, N, NC]). % piesa rotita o data
ccw(Tile, Rotation, RotatedTile) :- Rotation > 1, 
                                    ccw(Tile, 1, RTile), % piesa se roteste o data
                                    NewRotation is Rotation - 1, 
                                    ccw(RTile, NewRotation, RotatedTile). % piesa se roteste de Rotation - 1 ori
...
rot(Tile, Rotation, Acc, RotationPairs) :- ccw(Tile, Rotation, RTile), % piesa se roteste
                                        \+ member((X, RTile), Acc), % nu exista in Acc => se adauga
                                        NewRotation is Rotation + 1,
                                         % se apeleaza functia pana se intaleste o piesa deja adaugata
                                        rot(Tile, NewRotation, [(Rotation, RTile) | Acc], RotationPairs);
                                        ccw(Tile, Rotation, RTile),  % piesa se roteste
                                        member((X, RTile), Acc), % exista in Acc
                                        RotationPairs = Acc. % rezultatul final

rotations(Tile, RotationPairs) :- rot(Tile, 0, [], RotationPairs).
...
% elementul What se gaseste pe NeighborDirection in Tile si pe directia opusa in NeighbourTile
match(Tile, NeighborTile, NeighborDirection) :- at(Tile, NeighborDirection, What), 
                                                opposite(NeighborDirection, Direction),
                                                at(NeighborTile, Direction, What).
...
checkRotation([], _).
checkRotation([(N, NDir) | Neighbors], TileRotated) :- match(TileRotated, N, NDir), checkRotation(Neighbors, TileRotated).

iterateRotations(_, Acc, Rotation, []) :- Rotation = Acc.
iterateRotations(Neighbors, Acc, Rotation, [Pair | RotationPairs]) :- Pair = (X, TileRotated),
                                                                checkRotation(Neighbors, TileRotated),
                                                                iterateRotations(Neighbors, [Acc | X], Rotation, RotationPairs);
                                                                iterateRotations(Neighbors, Acc, Rotation, RotationPairs).

findRotation(Tile, Neighbors, Rotation) :- rotations(Tile, RotationPairs),
                                           iterateRotations(Neighbors, [], Rotation, RotationPairs).
...
boardSet(BoardIn, Pos, Tile, [(Tile, Pos)]) :- emptyBoard(BoardIn). % daca tabla e goala, se adauga piesa pe pozitia Pos
boardSet(BoardIn, Pos, Tile, BoardOut) :- \+ member((X, Pos), BoardIn), % nu exista in BoardIn => se adauga
                                        BoardOut = [(Tile, Pos) | BoardIn]. % se adauga piesa pe pozitia Pos
...
boardGet(Board, Pos, Tile) :- member((X, Pos), Board), Tile = X. % daca exista in Board, se leaga Tile la X
...
getLists([], [], []). 
getLists([X | XList], [Y | YList], [(Tile, (X, Y)) | Board]) :- getLists(XList, YList, Board). % se obtin listele de coordonate x si y

boardGetLimits([], _, _, _, _) :- false.
boardGetLimits(Board, XMin, Ymin, XMax, YMax) :- getLists(XList, YList, Board),
                                                max_list(XList, XMax), % se obtin coordonatele extreme
                                                min_list(XList, XMin), % se obtin coordonatele extreme
                                                max_list(YList, YMax), % se obtin coordonatele extreme
                                                min_list(YList, Ymin). % se obtin coordonatele extreme
...
findNeighbors(_, _, _, Acc, Neighbors, []) :- Neighbors = Acc.
findNeighbors(Board, Pos, Tile, Acc, Neighbors, [D | Directions]) :- neighbor(Pos, D, NeighborPos), % pozitia vecinului
                                                                    boardGet(Board, NeighborPos, NeighborTile), % exista piesa vecinului
                                                                    append(NewAcc, [(NeighborTile, D)], Acc), % adaug vecinul
                                                                    findNeighbors(Board, Pos, Tile, NewAcc, Neighbors, Directions);
                                                                    findNeighbors(Board, Pos, Tile, Acc, Neighbors, Directions).

checkNeighbors(Board, Pos, Tile) :- directions(Directions), % toate directiile
                                    findNeighbors(Board, Pos, Tile, [], Neighbors, Directions), % vecinii
                                    length(Neighbors, Len),
                                    Len > 0, % exista vecini
                                    forall(member((NeighborTile, D), Neighbors), match(Tile, NeighborTile, D)). % toti vecinii se potrivesc

canPlaceTile([], _, _).
canPlaceTile(Board, Pos, Tile) :- \+ boardGet(Board, Pos, Tile), % poziția este libera
                                    checkNeighbors(Board, Pos, Tile).
...
getNeighbors(_, [], []).
getNeighbors(B, [D | Directions], Neighbors) :- B = (Tile, Pos), % piesa si pozitia
                                                neighbor(Pos, D, NeighborPos), % pozitia vecinului
                                                \+ boardGet(Board, NeighborPos, NeighborTile), % nu exista piesa vecinului
                                                getNeighbors(B, Directions, RestNeighbors), 
                                                append(RestNeighbors, [(NeighborTile, NeighborPos)], Neighbors); % adaug vecinul
                                                getNeighbors(B, Directions, Neighbors).

getAvailablePositions([], _) :- false. % tabla goala

getAvailablePositions([B], Positions) :- directions(Directions), % toate directiile
                                        getNeighbors(B, Directions, Neighbors), % vecinii
                                        Positions = Neighbors. % se leaga Positions la vecini

getAvailablePositions([B | Board], Positions) :- \+ emptyBoard(Board), % tabla nu este goala
                                                directions(Directions), % toate directiile
                                                getNeighbors(B, Directions, Neighbors), % vecinii
                                                getAvailablePositions(Board, RestPositions), % vecinii de pe restul tablei
                                                append(Neighbors, RestPositions, Positions). % se leaga Positions la vecini
...
% (X,Y):Rotation.
findPositionForTile([], _, (0, 0), 0). % tabla goala
findPositionForTile(Board, Tile, Position, 0) :- getAvailablePositions(Board, Positions), % toate pozitiile disponibile
                                                member((Tile, Position), Positions), % exista o pozitie disponibila
                                                canPlaceTile(Board, Position, Tile). % se poate plasa piesa
